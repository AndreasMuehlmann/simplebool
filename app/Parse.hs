...
instance Eq BoolExpr where
  ...
  Conjunction leftLeftBoolExpr leftRightBoolExpr == Conjunction rightLeftBoolExpr rightRightBoolExpr =
    (leftLeftBoolExpr == rightLeftBoolExpr)
      && (leftRightBoolExpr == rightRightBoolExpr)
  Disjunction leftLeftBoolExpr leftRightBoolExpr == Disjunction rightLeftBoolExpr rightRightBoolExpr =
    (leftLeftBoolExpr == rightLeftBoolExpr)
      && (leftRightBoolExpr == rightRightBoolExpr)
  _ == _ = False

instance Show BoolExpr where
  show (Conjunction leftExpr rightExpr) =
    (if bracketsRequired leftExpr then "(" ++ show leftExpr ++ ")" else show leftExpr)
      ++ " and "
      ++ (if bracketsRequired rightExpr then "(" ++ show rightExpr ++ ")" else show rightExpr)
  show (Disjunction leftExpr rightExpr) =
    (if bracketsRequired leftExpr then "(" ++ show leftExpr ++ ")" else show leftExpr)
      ++ " or "
      ++ (if bracketsRequired rightExpr then "(" ++ show rightExpr ++ ")" else show rightExpr)
